...
func runLocalInstall(packages []*installer.Package) {
	...
	for i, pkg := range packages {
		...
		if err != nil {
			...
			fmt.Println(theme.ErrorStyle.Render(fmt.Sprintf("  💥 FAILED TO INSTALL: %s", pkg.Name)))
			fmt.Println(theme.ErrorStyle.Render("═══════════════════════════════════════════════"))
			fmt.Println()

			// Show progress summary
			if i > 0 {
				fmt.Println(theme.InfoStyle.Render(fmt.Sprintf("✓ Successfully installed %d/%d packages before failure:", i, len(packages))))
				for j := 0; j < i; j++ {
					fmt.Println(theme.DimTextStyle.Render(fmt.Sprintf("  • %s", packages[j].Name)))
				}
				fmt.Println()
			}

			fmt.Println(theme.WarningStyle.Render("💡 Need help fixing this?"))
			fmt.Println(theme.DimTextStyle.Render(fmt.Sprintf("   Package ID: %s", pkg.ID)))
			fmt.Println(theme.SuccessStyle.Render("   Run: anime doctor <log-file>"))
			fmt.Println(theme.DimTextStyle.Render("   Or pipe: anime install ... 2>&1 | anime doctor"))
			os.Exit(1)
		}
...
func runRemoteInstall(packages []*installer.Package) {
	...
	for i, pkg := range packages {
		...
		if err != nil {
			...
			fmt.Println(theme.ErrorStyle.Render("═══════════════════════════════════════════════"))
			fmt.Println(theme.ErrorStyle.Render(fmt.Sprintf("  💥 FAILED TO INSTALL: %s", pkg.Name)))
			fmt.Println(theme.ErrorStyle.Render(fmt.Sprintf("  📍 Server: %s", installServer)))
			fmt.Println(theme.ErrorStyle.Render("═══════════════════════════════════════════════"))
			fmt.Println()
			...
			// Show error output
			if len(output) > 0 {
				fmt.Println(theme.WarningStyle.Render("📋 Error output:"))
			...
			fmt.Println(theme.DimTextStyle.Render(fmt.Sprintf("   Package ID: %s", pkg.ID)))
			fmt.Println(theme.SuccessStyle.Render("   Run: anime doctor <log-file>"))
			fmt.Println(theme.DimTextStyle.Render("   Or save: anime install ... 2>&1 | tee error.log"))
			os.Exit(1)
		}
