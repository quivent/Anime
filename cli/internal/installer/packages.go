...
func GetPackages() map[string]*Package {
	return map[string]*Package{
		...
		"domain": {
			...
			Description:   "Points comfort.producer.cafe → server IP via Vercel DNS API, installs nginx reverse proxy + Let's Encrypt SSL certificate",
			Dependencies:  []string{"comfort"},
			EstimatedTime: 2 * time.Minute,
			Category:      "Networking",
			Size:          "~10MB",
		},
		"wan": {
			...
			Description:   "Complete Wan 2.2 video-gen stack: driver-aware torch, sage attn, Kijai Wan nodes, full Wan 2.2 model set (~85GB), Comfort studio UI + domain. Runs on any CUDA GPU with ≥16GB VRAM (5B preset) or ≥24GB (14B preset).",
			Dependencies:  []string{"comfyui", "wantorch", "wannodes", "wanmodels", "comfort", "domain"},
		...
		// One command: anime install llama
		// =========================================================================
		"llamamodels": {
			ID:            "llamamodels",
