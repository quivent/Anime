...
func NewConfigModel() (ConfigModel, error) {
	...
	// Auto-configure to localhost if no servers exist
	if len(cfg.Servers) == 0 {
		if err := autoConfigureLocalhost(cfg); err != nil {
			// Don't fail if auto-config fails, just continue with empty config
...
// autoConfigureLocalhost creates a default server entry for the local machine
func autoConfigureLocalhost(cfg *config.Config) error {
	// Get current user
	...
	// Create default server entry
	localServer := config.Server{
		Name:        "local",
		Host:        hostname,
		User:        currentUser.Username,
		SSHKey:      "",
		CostPerHour: 0.0, // Local machine has no cost
		Modules:     []string{},
	}

	cfg.AddServer(localServer)
