...
func runWizard(cmd *cobra.Command, args []string) {
	...
	if config.installOllama {
		printStep(2, "Which LLM models do you want?")
		fmt.Println()
		...
		fmt.Println(theme.DimTextStyle.Render("       Mistral, Llama 3.3 8B, Qwen 2.5 7B"))
		fmt.Println(theme.HighlightStyle.Render("  2") + theme.DimTextStyle.Render(" → ") + theme.InfoStyle.Render("⭐ Medium Models (14-34B)") + theme.DimTextStyle.Render(" - ~40GB, balanced"))
		fmt.Println(theme.DimTextStyle.Render("       Qwen 2.5 14B, Mixtral 8x7B, DeepSeek Coder 33B"))
		fmt.Println(theme.HighlightStyle.Render("  3") + theme.DimTextStyle.Render(" → ") + theme.InfoStyle.Render("⭐ Large Models (70B+)") + theme.DimTextStyle.Render(" - ~80GB, best quality"))
		fmt.Println(theme.DimTextStyle.Render("       Llama 3.3 70B, Qwen 2.5 72B, DeepSeek V3"))
		fmt.Println()
	...
	// Step 3: Video/Image Models
	if config.installComfyUI || config.installPyTorch {
		printStep(3, "Which video/image generation models?")
		fmt.Println()
	...
	// Step 4: Ollama Service Configuration
	if config.installOllama {
		printStep(4, "Ollama Service Configuration")
		fmt.Println()
	...
	printStep(5, "Cluster & Parallelization")
	fmt.Println()
